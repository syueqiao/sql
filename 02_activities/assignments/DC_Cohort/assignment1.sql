...
CASE product_qty_type
	...
	END AS prod_qty_type_condensed,
	CASE
	WHEN product_name LIKE UPPER('%pepper%') THEN '1' 
	ELSE '0' 
	END AS pepper_flag
FROM product
...
SELECT customer_id, sum(quantity * cost_to_customer_per_qty) AS spent_per_purchase,
strftime('%m',market_date) AS "month", 
strftime('%Y',market_date) AS "year"
FROM customer_purchases
...
SELECT customer_id, market_date, quantity * cost_to_customer_per_qty AS spent_per_purchase
FROM customer_purchases
ORDER BY customer_id, market_date 

/*In April 2022, customer 1 spent 152, and customer 2 spent 172.5*/
SELECT customer_id, sum(quantity * cost_to_customer_per_qty) AS spent_per_purchase,
strftime('%m',market_date) AS "month", 
strftime('%Y',market_date) AS "year"
FROM customer_purchases
WHERE month = '04' AND year = '2022' AND customer_id BETWEEN 1 AND 2
GROUP BY customer_id, month, year 
